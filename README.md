...
产品界面采用 `ydchen-portfolio` 的暖米白、浅灰与赤陶色视觉系统，使用衬线标题和扁平化工作区。工作区采用 16px 字号基线，模板与快捷操作标签不小于 13px；画布浮层必须满足浅色背景对比度，桌面和手机端不得出现页面整体横向溢出。
...
手机端模板和样本选择面板可折叠，初始收起；画布操作与全屏提示使用独立高对比颜色。

首页移除占据右侧的大幅装饰图，数据入口和示例入口集中展示；实际作品画布、旋转预览和导出功能保留。

首页移除纯装饰的大幅演示图，保留工作室作品预览和全部导出方式。工作室顶部工具栏随页面滚走；新增重复本地导入、预设恢复/存储失败及 PNG/SVG/manifest/ZIP 导出浏览器回归。
